	...
	fn gas_to_weight(gas: u64, without_base_weight: bool) -> Weight {
		...
		if without_base_weight {
			weight = weight.saturating_sub(
				u64::try_from(T::BlockWeights::get()
					.get(frame_support::weights::DispatchClass::Normal)
					.base_extrinsic.ref_time()).unwrap_or(u32::MAX as u64),
			);
		}
		Weight::from_ref_time(weight)
	}
	fn weight_to_gas(weight: Weight) -> u64 {
		u64::try_from(weight.ref_time().wrapping_div(T::WeightPerGas::get())).unwrap_or(u32::MAX as u64)
	}
}
